...
testCoords =
  let from :: Coords Pos
      ...
      to = Coords 1 0
      d = distance from to
      e = mkEvolutionEaseQuart (Successive [from, to]) (fromSecs 1)
...
testSuccessiveInts =
  let s = Successive [3,7,9,5]
      e = mkEvolutionEaseQuart s $ fromSecs 1
...
testClock =
  let lastFrame = Frame 10
      (EaseClock clock) = mkEaseClock (fromSecs 1) lastFrame invQuartEaseInOut
