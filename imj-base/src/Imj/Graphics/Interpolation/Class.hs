...
module Imj.Graphics.Interpolation.Class
...
* interpolation /by value/ ('interpolate'), i.e the result of the interpolation
between two \(v\) is a \(v\),
* 'MonadIO' interpolation ('interpolateIO'), i.e if the result of the interpolation
between two \(v\) is an 'MonadIO' action.

The instances of this class should statisfy the following constraints (the
constraints are expressed in terms of the /by value/ interpolation, but I think
the same /ideas/ could be /projected/ on the monadic case, by using an abstract
notion of /drawing/ to represent the result of the 'MonadIO' action):
...
class (Show v) => DiscretelyInterpolable v where
  ...
  -- | Implement this function if you want to interpolate in 'MonadIO', i.e if
  -- the result of the interpolation between two \(v\) is a 'MonadIO' action.
