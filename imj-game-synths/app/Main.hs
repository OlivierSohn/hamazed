...
-- Note that the server will also send 'PlayMusic' events, which are generic events.
              ...
              (SynthsClientView _ _ _ idx) <- adjustClient
                (\(SynthsClientView _ _ loops loopIdx) ->
                  SynthsClientView
                    ...
                    $ loopIdx + 1)
              return
                [ (\v -> forever $ playLoopOnce -- TODO make loops cancelable.
                    (\a -> modifyMVar_ v . execStateT . playLoopMusic (toSrv e) (LoopId creator $ idx-1) pianoV a)
                    l)
                    ]))
   where

    onRecordableNote :: (MonadIO m, MonadState (ServerState SynthsServer) m, MonadReader ConstClientView m)
                       => Music -> Instrument -> m [ServerEvent SynthsServer]
...
toSrv :: ClientEventT SynthsServer -> Proxy SynthsServer
toSrv _ = Proxy
