...
Where does the event come from? What are we allowed / not allowed to do with a ClientOnlyEvtT that we could do
with a ServerEvent?
If it is ultimately the same thing, except that one is generated by the server, another generated by the client,
